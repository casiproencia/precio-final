...
document.getElementById("upload").addEventListener("change", async (e) => {
  ...
  const step = image.height / 6;
  const precios = [];

  for (let i = 0; i < 6; i++) {
    const sy = i * step;
    const segment = ctx.getImageData(image.width * 0.55, sy, image.width * 0.4, step);
  ...
  if (precios.length === 6) {
    ...
    tabla.innerHTML = `
      ...
      <tbody>
        ${precios.map(f => `<tr><td>${f[0]}</td><td>${f[1]}</td><td>${f[2]}</td></tr>`).join("")}
      </tbody>
    `;
